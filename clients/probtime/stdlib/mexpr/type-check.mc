...
lang TypeCheck = TCUnify + Generalize + RemoveMetaVar
  ...
  -- | tm ->
  --   dprint tm; print "typeCheckExpr error\n";
  --   -- print ((compose int2string constructorTag) tm);
  --   error ((compose int2string constructorTag) tm);
  --   tm
end
      ...
      let lty =
        if env.disableConstructorTypes then lty
          ...
          match lty with TyAll (r & {kind = Data d}) then
            ...
            createSingleError [t.info] "* Invalid constructor type in typeCheckExpr!";
            TyUnknown { info=t.info }
            -- TmConApp {t with body = body, ty = tyunknown_}
            -- error "Invalid constructor type in typeCheckExpr!"
      in
  ...
  | PatCon t ->
    match mapLookup t.ident env.conEnv with Some (_, ty) then
      match inst t.info env.currentLvl ty with TyArrow {from = from, to = to} then
        ...
        createSingleError [t.info] "Invalid constructor type in typeCheckPat!";
        (patEnv, PatCon {t with ty = tyunknown_})
        -- error "Invalid constructor type in typeCheckPat!"
    else
      let msg = join [
        "* Encountered an unbound constructor: ",
        nameGetStr t.ident, "\n",
