    ...
    match mapLookup (nameGetStr t.ident) env.langEnv with Some langEnv 
      then 
        TmUse {
          t with
          -- ident =
          inexpr = symbolizeExpr (updateEnv env langEnv) t.inexpr,
          langEnv = langEnv
        }
      else 
        symLookupError 
      ...
      (lam incl. match mapLookup (nameGetStr incl) env.langEnv 
                 with Some langEnv then langEnv
                  ...
                  if deref __LSP__SOFT_ERROR then
                    ...
                    let err = join ["The included language '", nameGetStr incl, "' can not be found!"] in
                    modref __LSP__BUFFERED_ERRORS (concat errs [(t.info, err)]);
                    _nameEnvEmpty
                  else
                    errorMulti [(t.info, nameGetStr incl)] "The included language can not be found!")
    t.includes in 
